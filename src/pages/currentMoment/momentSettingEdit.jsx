...
const MomentSettingNew = () => {
    ...
    const getMomentData = () => {
        fetch('/api/momentsetting?id='+momentID, {
            method: 'GET',
            headers: {
            ...
            .then(data => {
                ...
                const weekArray = [...new Set(momentData.group_week.split(','))]
                const timeArray = [...new Set(momentData.group_time.split(','))]
                form.setFieldsValue({
    ...
    const handleOk = () => {
        ...
        fetch('/api/momentsetting', {
            ...
            body: JSON.stringify({ DataInfo: JSON.stringify(formData), type: 'edit', App_moment_id: momentID })
        })
            .then(res => { if (!res.ok) { throw new Error(`API error - Status: ${res.status}`); } return res.json(); })
            .then(data => {
                setOpen(false);
                setConfirmLoading(false);
                messageBox("success", "編輯當下時刻時段成功!")
                console.log(data)
            })
            .catch(error => {
                messageBox("error", "編輯當下時刻時段失敗!")
    ...
    const formTable = (
        ...
        >
            ...
            >
                <Button type="primary" htmlType="submit" shape="round" icon={<PlusOutlined />} size='large'>編輯</Button>
            </Form.Item>
        </Form>
    )
    return (
